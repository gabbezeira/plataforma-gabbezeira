...
export function VideoUpdate({ onClose, videoData, isEditing }) {
	...
	useEffect(() => {
		if (isEditing && videoData) {
			...
			setThumb(videoData.thumb)
			setFiles(videoData.files)
		}
		setIsSnackbarActive(snackbar.show)
	}, [snackbar, isEditing, videoData])

	const handleSubmit = async (e) => {
		e.preventDefault()
		const data = { title, description, link, thumb, files }
		try {
			if (isEditing && videoData?.numericId) {
				await api.put(`/videos/${videoData.numericId}`, data)
				showSnackbar('Vídeo Atualizado!', 'success')
			} else {
			...
			showSnackbar('Erro ao salvar vídeo:', error.message, 'error')
		}
	}
	...
	return (
		<Container>
			<h1 className="title">
				{isEditing ? 'Editar vídeo' : 'Cadastrar vídeo'}
			</h1>
